...
class MadelineProtoApi extends Service
{
    ...
    /**
     * 获取当前用户对话列表
     * @param bool $account_id      用户账号chat_id
     * @param bool $exclude_pinned  排除固定对话
     * @param int $folder_id        默认文件夹
     * @param int $offset_date      分页偏移
     * @param int $offset_id        消息 ID 偏移
     * @param null $offset_peer     偏移对话
     * @param int $limit            数量限制
     * @return array
     * @throws Exception
     */
    public static function getDialogs($account_id,$exclude_pinned = true,$folder_id = 0,$offset_date = 0,$offset_id = 0,$offset_peer = null,$limit = 20)
    {
        $madeline = self::initMadelineProto($account_id);
